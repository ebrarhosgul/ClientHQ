...
import InvoiceError from "./error";

describe("InvoiceError", () => {
  it("announces the failure rather than showing a stack trace", () => {
    render(<InvoiceError error={new Error("boom")} reset={vi.fn()} />);
  ...
  it("retries through reset, not a full navigation", async () => {
    ...
    const reset = vi.fn();
    render(<InvoiceError error={new Error("boom")} reset={reset} />);
  ...
  it("offers a way back to the invoices list", () => {
    render(<InvoiceError error={new Error("boom")} reset={vi.fn()} />);
  ...
  it.each(THEMES)(
    ...
    async (theme) => {
      const { container } = render(
        <InvoiceError error={new Error("boom")} reset={vi.fn()} />,
      );
