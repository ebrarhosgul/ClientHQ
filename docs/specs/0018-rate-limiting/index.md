...
- **AC-8**: Every refusal, whichever caller asked, is logged by the door itself as one structured line `event: "rate_limit.refused"` with `subject`, `action`, `count`, `limit` and `window_start`, written as `console.warn(JSON.stringify(line))` in the shape of `src/auth/webhook-log.ts`. Neither log line carries a name, an email address or any input field; the subject is an id.
...
| `consume` | `window_start` | derived: `windowStart(now, policy.windowSeconds)` (AC-4) |
| `consume` | `count` after this attempt | the upsert's `returning` (AC-3) |
| `consume` | allowed or refused | derived: `count <= limit` |
| `consume` | `retryAfterSeconds` | derived: `window_start + windowSeconds - now`, in whole seconds rounded up |
...
| `consume` | the refusal log fields | `subject`, `action`, `count`, `limit`, `window_start` from the verdict it just built, logged by the door before returning (AC-8) |
