...
2. [ ] Thin thread: replace the `/team` placeholder with a Server Component that reads the membership list through `src/team/queries.ts` and renders the Members table (person, `You` badge, role, joined), plus the invite card and the `inviteTeamMember` action with the Zod input, the role mapping and the `/sign-up` redirect URL. Deploy, send a real invitation to a second mailbox, accept it through `/sign-up`, land on `/onboarding`, and confirm the new member appears; repeat with an address that already has an account. Satisfies **AC-1** (members view), **AC-2**.
   - Built: `/team` renders the members table and the invite card through `src/team/queries.ts`, `src/team/ui/team-page-view.tsx` and `src/team/invite-team-member.ts`, with the `/sign-up` redirect. **Not yet proven on the deployed app**: the real invitation email, the ticket landing on `/sign-up`, the existing account bounce to `/sign-in`, and `/onboarding` activating the agency need a deploy and a second mailbox, which is the engineer's walk (see `verify.md`).
3. [x] Pending invitations: read the pending list, render the Pending invitations table for admins, add `revokeTeamInvitation` with the `not_found` resolution against the list. Satisfies **AC-1** (pending view), **AC-4**.
   - Proven: the pending table with the empty state, `revokeTeamInvitation` resolving the id against the organization's own pending list (`not_found` otherwise), any admin may revoke. Action tests in `src/team/actions.test.ts`.
4. [x] Duplicate checks in `inviteTeamMember`: the pure `duplicateEmail` rule in `rules.ts` over the two full lists, returning the two `conflict` messages, plus the mapping of Clerk's own duplicate refusal to the already invited `conflict`. Satisfies **AC-3**.
   - Proven: `duplicateEmail` in `rules.ts` over both full lists, the two `conflict` messages as field errors on `email`, and Clerk's own duplicate refusal mapped to already invited. Rules and action tests.
5. [x] Role change: `changeTeamMemberRole` with the `not_found` resolution, the no op on the same role, the pure `lastAdminBlocks` rule, the Clerk update, and the write through to `memberships.role` through `tenantDb(ctx)`; the per row role select with pending state and the `aria-live` result. Satisfies **AC-5**, **AC-14** (select).
   - Proven: `changeTeamMemberRole` with `findMembership`, the no op, `lastAdminBlocks`, the Clerk write with the user id taken from the list, the write through in `src/team/mirror.ts`; `MemberRoleSelect` with the pending state, the `aria-live` status line and the locked own row.
6. [x] Removal: `removeTeamMember` with the last admin rule, the Clerk delete, the hard delete of the `memberships` row only (never `users`), `self`, and the accessible confirm dialog with the two wordings. Client handling of self actions: set the active organization to none and go to `/onboarding` after leaving; reactivate the same organization and refresh after self demotion. Satisfies **AC-6**, **AC-7**, **AC-14** (dialog).
   - Proven: `removeTeamMember`, the mirror hard delete of the `memberships` row only, `self`, `RemoveMemberButton` with both wordings, `ClerkSessionSync` setting the active organization to none then `/onboarding` after leaving, and reactivating the same organization after a self demotion. Component tests in `src/team/ui/remove-member-button.test.tsx`.
7. [x] Half done handling and logging: wrap each mirror write so a failure after a Clerk success logs at error level and still returns `ok`; add the structured log line to every action outcome with the target and conflict kind of AC-13 and no email address. Satisfies **AC-10**, **AC-13**.
   - Proven: a mirror failure after a Clerk success logs `mirror_failed` at error level and still returns `ok`; `src/team/log.ts` writes one `team.management` line per outcome with the AC-13 target rules; the action tests assert no line contains `@`.
8. [x] Harden spec 0005's repair path in `src/auth/context.ts`: add `agencyMemberships()` to the existing `Promise.all`, confirm a membership for `clerkOrgId` before `ensureMirrorRows`, take the role from it, redirect to `/onboarding` when absent; update `src/auth/context.test.ts` for the removed person case. Satisfies **AC-12**.
   - Proven: `repairMirror` runs the three Clerk reads together, takes the role from the membership, and redirects to `/onboarding` when Clerk lists no membership for the organization. `src/auth/context.test.ts` covers the stale admin claim and the removed person.
9. [x] Error and member states: the Clerk error card with `Try again` on the page, the three `unavailable` messages in the actions, the member view with its one line note, the empty pending state, and an accessibility pass over every state. Satisfies **AC-11**, **AC-1**, **AC-14**.
   - Proven: the error card with `Try again`, the three `unavailable` messages in `src/team/clerk-failure.ts`, the member view with its note, the empty pending state; axe passes in both themes over the admin, solo admin, member and error states (`src/team/ui/team-page-view.test.tsx`), and the gallery section renders every state at `/design`.
10. [x] Confirm the gate and the guards: declare all four actions with `requireRole: "admin"` and the default subscription setting, and test a member and a grace window agency against each. Satisfies **AC-8**.
   - Proven: all four actions declare `requireRole: "admin"` and no `subscription` opt out (asserted in `actions.test.ts` from the recorded configs); the wrapper's own guard and gate tests already cover the refusals.
11. [x] Update the `/settings` placeholder comment to point at a later settings feature. Satisfies **AC-15**.
   - Proven: the `/settings` placeholder comment now points at a later settings feature.
12. [ ] Tests per the GA workflow: Vitest on `rules.ts` (last admin, duplicates, role mapping), action tests with a stubbed `src/auth/clerk.ts` covering every scenario above, a page test for the admin and member views and the error card, and Playwright for the page rendering as admin and as member (the real invitation email stays a manual check on the deployed app, as spec 0005 did for sign up). Satisfies **AC-1** to **AC-14**.
   - Partly: Vitest covers the rules, the Clerk wrappers, the four actions, the mirror helpers, the page view (with axe) and the remove button, 66 tests in `src/team`. **Not yet**: a Playwright walk of `/team` as admin and as member, which needs a signed in staff user in the browser suite (today only the contact user is provisioned), and the deployed app walk of the invitation. Left for `/test` and `/check verify`.
