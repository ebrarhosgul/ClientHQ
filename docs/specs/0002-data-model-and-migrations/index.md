...
8. Add `invoices.ts`: `invoices` and `invoice_line_items`, with the money columns, `currency` and its format CHECK, `tax_rate_bp`, `quantity` as `numeric({ precision: 12, scale: 3, mode: 'string' })`, `position` 1 based and unique per invoice, the (`org_id`, `number`) unique constraint, the (`org_id`, `status`, `due_date`) sweep index, and every CHECK including `amount_cents = round(quantity * unit_amount_cents)` and `tax_cents = round(subtotal_cents * tax_rate_bp / 10000.0)`, satisfies **AC-3**, **AC-4**, **AC-5**
9. Add `webhooks.ts`: `processed_webhook_events` with the (`source`, `event_id`) unique constraint and the `processed_at` index. Declare `relations()` for every table and re export everything from `index.ts`, satisfies **AC-1**
10. Delete the interim migration and its snapshot, run `pnpm db:generate` once to produce the single baseline, apply it to a fresh database and confirm the container job passes on it. This squash is safe **only** because nothing is deployed yet, satisfies **AC-1**, **AC-7**, **AC-8**
