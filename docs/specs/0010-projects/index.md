...
| the move button | what refreshes the page after a move | `ProjectStatusActions` calls `router.refresh()` after the action returns, on success and on `conflict` alike (where `archive-client-button.tsx` refreshes on success only); the action's `revalidate` config runs on success only, because `withTenantAction` skips it when the handler throws. The error sits in the group's own state beside the buttons, not inside the button that was clicked, and the detail page keeps the group mounted while archived: every conflict changes the set of buttons, so a message held by a button would vanish with the refresh that AC-9 asks for |
...
| `/clients/[id]` Projects section | the New project link target | `/projects/new?client=<id>`, and no link at all for an archived client: the create picker never lists one (AC-3), so the section says the client is archived instead, as the contacts section does |
