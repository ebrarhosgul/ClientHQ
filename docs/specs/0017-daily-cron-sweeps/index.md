...
- **AC-9**: `retention_prune` deletes `processed_webhook_events` rows with `processed_at < now() - interval '90 days'` and `cron_runs` rows with `started_at < now() - interval '90 days'`, reporting both counts. The current run's row is never deleted. Amended by spec 0018: the sweep also deletes `rate_limit_windows` rows with `window_start < now() - interval '7 days'`, reported as `rate_limit_windows_pruned`.
